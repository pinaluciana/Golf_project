...
# Imports below require src/ to be in path first
...
from models.evaluation import (
    evaluate_ml_models,
    print_econometric_summary,
    evaluate_econometric_models,
    print_ml_summary,
)
...
def main():
    ...
    logger.info(
        "  Removed players who didn't make the cut, got disqualified (DQ), or withdrawn (WD)"
    )
    ...
    for group_name, features in groups.items():
        ...
        logger.info("  %s: %s", group_name.upper().replace("_", " "), feature_list)
    ...
    print()  # Blank line for readability
    logger.info("FINAL FEATURES USED IN MODELS:")
    logger.info("FINAL FEATURE SET (%d features):", len(feature_engineering.FEATURES))
    for i, feature in enumerate(feature_engineering.FEATURES, 1):
        logger.info("  %2d. %s", i, feature)
