...
def evaluate_econometric_models(econ_results, data):
    ...
    pooled_metrics = compute_regression_metrics(
        econ_results["pooled_linear"]["y_true"], econ_results["pooled_linear"]["y_pred"]
    )

    logger.info("Pooled Linear Regression:")
    logger.info(f"  R²:          {pooled_metrics['r_squared']:.4f}")
    logger.info(
        f"  Adjusted R²: {econ_results['pooled_linear']['model'].rsquared_adj:.4f}"
    )
    ...
    for major in data["major"].unique():
        ...
        logger.info(
            f"  {major:<25} R² = {major_model.rsquared:.4f}, Adj R² = {major_model.rsquared_adj:.4f}, RMSE = {major_metrics['rmse']:.4f} strokes, n = {major_metrics['n_observations']}"
        )
...
def print_econometric_summary(econ_results, data, results_dir_econ):
    ...
    print("-" * 100)
    print(
        "SECTION 4.1: Pooled Linear Regression Results (explain score based on performance metrics)"
    )
    print("-" * 100)
    ...
    top_coefs = econ_results["pooled_linear"]["coefficients"][
        econ_results["pooled_linear"]["coefficients"]["Feature"] != "const"
    ].copy()
    top_coefs = top_coefs.reindex(
        top_coefs["Coefficient"].abs().sort_values(ascending=False).head(5).index
    )
    for _, row in top_coefs.iterrows():
        logger.info(
            f"  {row['Feature']:<15} {row['Coefficient']:>7.3f}  (p={row['p_value']:.3f})"
        )

    # Section 4.2: Per-Major Linear Regression Results
    print("\n" + "-" * 100)
    print(
        "SECTION 4.2: Per-Major Linear Regression Results (show how skill importance varies across tournaments)"
    )
    print("-" * 100)
    for major in data["major"].unique():
        ...
        top_3 = major_coefs.reindex(
            major_coefs["Coefficient"].abs().sort_values(ascending=False).head(3).index
        )
    ...
    print("-" * 100)
    print(
        "SECTION 4.3: Pooled Logistic Regression Results (probability of finishing in the top 25% of the leaderboard )"
    )
    print("-" * 100)
    ...
    top_logistic = econ_results["pooled_logistic"]["coefficients"][
        econ_results["pooled_logistic"]["coefficients"]["Feature"] != "const"
    ].copy()
    top_logistic = top_logistic.reindex(
        top_logistic["Coefficient"].abs().sort_values(ascending=False).head(5).index
    )
    ...
    print("\n" + "-" * 100)
    print(
        "SECTION 4.4: Logistic Regression with Interactions Results (probability of finishing in the top 25% of the leaderboard in each Major)"
    )
    print("-" * 100)
    inter_acc = accuracy_score(
        econ_results["interaction_logistic"]["y_true"],
        econ_results["interaction_logistic"]["y_pred"],
    )
...
def evaluate_ml_models(ml_results, results_dir):
    ...
    # Training metrics
    rf_train_pred = ml_results["random_forest"]["model"].predict(
        ml_results["random_forest"]["X_train"]
    )
    rf_train_proba = ml_results["random_forest"]["model"].predict_proba(
        ml_results["random_forest"]["X_train"]
    )[:, 1]
    rf_train_metrics = compute_classification_metrics(
        ml_results["random_forest"]["y_train"], rf_train_pred, rf_train_proba
    )
    ...
    logger.info(
        f"  Overfitting Gap:    {rf_train_metrics['accuracy'] - rf_test_metrics['accuracy']:.4f}"
    )
    print()  # Blank line
    ...
    cm_rf = compute_confusion_matrix(
        ml_results["random_forest"]["y_test"], ml_results["random_forest"]["y_pred"]
    )
    total_rf = cm_rf.sum()
    ...
    logger.info("  Actual")
    logger.info(
        f"  Rest              {cm_rf[0,0]:>6}     {cm_rf[0,1]:>6}    (True Negative: {cm_rf[0,0]}, False Positive: {cm_rf[0,1]})"
    )
    logger.info(
        f"  Top 25%           {cm_rf[1,0]:>6}     {cm_rf[1,1]:>6}    (False Negative: {cm_rf[1,0]}, True Positive: {cm_rf[1,1]})"
    )
    print()  # Blank line
    logger.info(
        f"Correctly Predicted: {cm_rf[0,0] + cm_rf[1,1]} out of {total_rf} ({(cm_rf[0,0] + cm_rf[1,1])/total_rf*100:.1f}%)"
    )
    ...
    # Training metrics
    xgb_train_pred = ml_results["xgboost"]["model"].predict(
        ml_results["xgboost"]["X_train"]
    )
    xgb_train_proba = ml_results["xgboost"]["model"].predict_proba(
        ml_results["xgboost"]["X_train"]
    )[:, 1]
    xgb_train_metrics = compute_classification_metrics(
        ml_results["xgboost"]["y_train"], xgb_train_pred, xgb_train_proba
    )
    ...
    logger.info(
        f"  Overfitting Gap:    {xgb_train_metrics['accuracy'] - xgb_test_metrics['accuracy']:.4f}"
    )
    print()  # Blank line
    ...
    print()  # Blank line
    logger.info("Confusion Matrix:")
    cm_xgb = compute_confusion_matrix(
        ml_results["xgboost"]["y_test"], ml_results["xgboost"]["y_pred"]
    )
    ...
    logger.info("  Actual")
    logger.info(
        f"  Rest              {cm_xgb[0,0]:>6}     {cm_xgb[0,1]:>6}    (True Negative: {cm_xgb[0,0]}, False Positive: {cm_xgb[0,1]})"
    )
    logger.info(
        f"  Top 25%           {cm_xgb[1,0]:>6}     {cm_xgb[1,1]:>6}    (False Negative: {cm_xgb[1,0]}, True Positive: {cm_xgb[1,1]})"
    )
    print()  # Blank line
    logger.info(
        f"Correctly Predicted: {cm_xgb[0,0] + cm_xgb[1,1]} out of {total_xgb} ({(cm_xgb[0,0] + cm_xgb[1,1])/total_xgb*100:.1f}%)"
    )
...
def print_ml_summary(ml_results, results_dir):
    ...
    xgb_metrics = compute_classification_metrics(
        ml_results["xgboost"]["y_test"],
        ml_results["xgboost"]["y_pred"],
        ml_results["xgboost"]["y_pred_proba"],
    )
    ...
    for metric in ["accuracy", "precision", "recall", "f1_score", "roc_auc"]:
        ...
        winner = (
            "XGBoost"
            if xgb_val > rf_val
            else "Random Forest" if rf_val > xgb_val else "Tie"
        )
    ...
    shap_top = ml_results["shap"]["shap_importance"][
        ~ml_results["shap"]["shap_importance"]["Feature"].str.startswith("major_")
    ].head(5)
    ...
    rf_top = ml_results["random_forest"]["importance"][
        ~ml_results["random_forest"]["importance"]["Feature"].str.startswith("major_")
    ].head(5)
    xgb_top = ml_results["xgboost"]["importance"][
        ~ml_results["xgboost"]["importance"]["Feature"].str.startswith("major_")
    ].head(5)
    shap_top = ml_results["shap"]["shap_importance"][
        ~ml_results["shap"]["shap_importance"]["Feature"].str.startswith("major_")
    ].head(5)
