...
def plot_pooled_linear_coefficients(coefficients_df, save_path):
    ...
    ax.axvline(
        x=0, color="black", linestyle="--", linewidth=0.8
    )  # Add a horizontal like at zero for reference
    ax.set_xlabel("Standardized Coefficient (Negative = Improves Score)", fontsize=12)
    ax.set_title(
        "Pooled Linear Regression: Feature Coefficients\n(Blue = Significant at p<0.05)",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_per_major_coefficient_heatmap(comparison_df, save_path):
    ...
    avg_abs_coef = comparison_df.abs().mean(
        axis=1
    )  # Compute the average absolute coefficient for each variable across Majors
    comparison_df = comparison_df.loc[
        avg_abs_coef.sort_values(ascending=False).index
    ]  # Reaorder the rows so that the most impactful variables appear first.
    ...
    plt.title(
        "Per-Major Linear Regression: Coefficient Comparison\n(Features Ordered by Average Impact)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_pooled_logistic_coefficients(coefficients_df, save_path):
    ...
    ax.set_xlabel(
        "Logistic Coefficient (Positive = Increases Top 25% Probability)", fontsize=12
    )
    ax.set_title(
        "Pooled Logistic Regression: Feature Coefficients\n(Predicting Top 25%, Blue = Significant at p<0.05)",
        fontsize=14,
        fontweight="bold",
    )
    ax.grid(axis="x", alpha=0.3)  # Add a light grid to make the chart easier to read
...
def plot_interaction_logistic_heatmap(comparison_table, save_path):
    ...
    avg_abs_coef = comparison_table.abs().mean(
        axis=1
    )  # axis=1 means average across columns (majors)
    comparison_table = comparison_table.loc[
        avg_abs_coef.sort_values(ascending=False).index
    ]  # Take the average across Majors
    ...
    plt.title(
        "Logistic Regression with Interactions: Major-Specific Effects\n(How Features Affect Top 25% Probability Per Major)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def create_econometric_visualizations(econ_results, results_dir):
    ...
    plot_pooled_linear_coefficients(
        econ_results["pooled_linear"]["coefficients"],
        save_path=figures_dir / "1_pooled_linear_coefficients.png",
    )

    # 2. Per-Major Linear Regression
    plot_per_major_coefficient_heatmap(
        econ_results["per_major_linear"]["coefficient_comparison"],
        save_path=figures_dir / "2_per_major_coefficient_heatmap.png",
    )

    # 3a. Pooled Logistic Regression
    plot_pooled_logistic_coefficients(
        econ_results["pooled_logistic"]["coefficients"],
        save_path=figures_dir / "3a_pooled_logistic_coefficients.png",
    )

    # 3b. Logistic Regression with Interactions
    plot_interaction_logistic_heatmap(
        econ_results["interaction_logistic"]["comparison_table"],
        save_path=figures_dir / "3b_interaction_logistic_heatmap.png",
    )
...
def plot_feature_importance_comparison(
    rf_importance, xgb_importance, shap_importance, save_path
):
    ...
    plt.suptitle(
        "ML Models and SHAP Feature Importance Comparison",
        fontsize=15,
        fontweight="bold",
        y=0.98,
    )
...
def plot_per_major_shap_heatmap(shap_per_major_df, save_path):
    ...
    shap_data.columns = [
        col.replace("SHAP_", "").replace("_", " ") for col in shap_data.columns
    ]
    ...
    plt.title(
        "SHAP Feature Importance by Major Championship\n(How Performance Metrics Drive Top 25% Predictions Per Major)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_confusion_matrices(rf_results, xgb_results, save_path):
    ...
    labels_rf = np.array(
        [
            ...
            for row_counts, row_pcts in zip(cm_rf, cm_rf_pct)
        ]
    )
    labels_xgb = np.array(
        [
            [f"{count}\n({pct:.1f}%)" for count, pct in zip(row_counts, row_pcts)]
            for row_counts, row_pcts in zip(cm_xgb, cm_xgb_pct)
        ]
    )
    ...
    sns.heatmap(
        ...
        cmap="Blues",
        ax=ax1,
        xticklabels=["Rest", "Top 25%"],
        yticklabels=["Rest", "Top 25%"],
        cbar_kws={"label": "Count"},
    )
    ax1.set_title(
        f"Random Forest\nConfusion Matrix\n(Overall Accuracy: {acc_rf:.1f}%)",
        fontsize=13,
        fontweight="bold",
    )
...
def create_ml_visualizations(ml_results, results_dir):
    ...
    plot_feature_importance_comparison(
        ml_results["random_forest"]["importance"],
        ml_results["xgboost"]["importance"],
        ml_results["shap"]["shap_importance"],
        save_path=figures_dir / "1_feature_importance_comparison.png",
    )

    # 2. Per-Major SHAP Heatmap (load the csv)
    shap_csv_path = results_dir / "4_shap_per_major_comparison.csv"
    shap_df = pd.read_csv(shap_csv_path)
    plot_per_major_shap_heatmap(
        shap_df, save_path=figures_dir / "2_per_major_shap_heatmap.png"
    )
    ...
    plot_confusion_matrices(
        ...
        ml_results["xgboost"],
        save_path=figures_dir / "3_confusion_matrices.png",
    )
