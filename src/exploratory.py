...
def add_top25_flag(df):
    ...
    df["tournament_25th_percentile"] = df.groupby(["major", "year"])[
        "total_score"
...
def compare_top25_vs_rest(df):
    ...
    return {
        ...
        "difference": (
            top_performers[KEY_METRICS].mean() - rest_of_field[KEY_METRICS].mean()
        ).round(3),
        "counts": {
...
def plot_top25_comparison(std_difference, save_path=None):
    ...
    plt.title(
        "Top 25% vs Rest: Standardized Differences",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_performance_heatmap(df, save_path=None):
    ...
    plt.title(
        "Performance Metrics by Major (Z-Scores)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_distribution_dashboard(df, save_path=None):
    ...
    plt.suptitle(
        "Performance Metrics Distribution by Major",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
...
def run_exploratory_analysis(df, results_dir=None):
    ...
    plot_correlation_heatmap(
        results["correlations"], save_path=figures_dir / "correlation_heatmap.png"
    )
    ...
    plot_distribution_dashboard(
        df, save_path=figures_dir / "distribution_dashboard.png"
    )
    ...
    plot_top25_comparison(
        results["top25_std_diff"], save_path=figures_dir / "top25_comparison.png"
    )
...
def print_exploratory_summary(df, exploratory_results, results_dir):
    ...
    winners = (
        df[df["position"] == "1"]
        .groupby("major")["total_score"]
        .agg(["mean", "min", "max"])
    )
    for major in winners.index:
        logger.info(
            f"  {major:<25} Mean: {winners.loc[major, 'mean']:>6.2f}  "
    ...
    logger.info(
        f"  Top 25%:       {top25_count['top_25']:>4} players ({top25_count['top_25']/top25_count['total']*100:.1f}%)"
    )
    logger.info(
        f"  Rest of field: {top25_count['rest']:>4} players ({top25_count['rest']/top25_count['total']*100:.1f}%)"
    )
    ...
    score_corr = (
        exploratory_results["correlations"]["total_score"]
        .drop("total_score")
        .abs()
